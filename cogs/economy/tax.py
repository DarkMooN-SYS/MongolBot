...
def calculate_tax(balance: int) -> int:
    ...
    """
    brackets = [
        (0, 100_000_000, 0.00),           # 0-100M: 0%
        (100_000_000, 200_000_000, 0.02), # 100M-200M: 2%
        (200_000_000, 500_000_000, 0.04), # 200M-500M: 4%
        (500_000_000, 1_000_000_000, 0.06), # 500M-1B: 6%
        (1_000_000_000, 2_000_000_000, 0.08), # 1B-2B: 8%
        (2_000_000_000, 5_000_000_000, 0.12), # 2B-5B: 12%
        (5_000_000_000, float('inf'), 0.15)   # 5B+: 15%
    ]
    ...
    try:
        ...
        for (user_id,) in users:
            try:
                ...
                if total_user_wealth < 100_000_000:
                    wealth_distribution['0-100M'] += 1
                elif total_user_wealth < 200_000_000:
                    wealth_distribution['100M-200M'] += 1
...
def test_tax_calculations():
    ...
    test_cases = [
        ...
        (100_000_000, 0),         # 100M₮ → 0₮ татвар (0-100M-ын 0%)
        (150_000_000, 1_000_000), # 150M₮ → 1M₮ татвар (100M-150M-ын 2% = 50M * 2%)
        (300_000_000, 6_000_000), # 300M₮ → 6M₮ татвар (100M*2% + 100M*4% = 2M + 4M)
        (1_000_000_000, 44_000_000), # 1B₮ → 44M₮ татвар (2M+12M+30M)
        (5_000_000_000, 484_000_000), # 5B₮ → 484M₮ татвар (2M+12M+30M+80M+360M)
        (10_000_000_000, 1_234_000_000), # 10B₮ → 1,234M₮ татвар
    ]
    
    print("🧮 ТАТВАРЫН ТООЦООЛЛЫН ТЕСТ:")
    print("=" * 50)
