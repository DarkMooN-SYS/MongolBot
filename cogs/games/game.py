...
class Game(commands.Cog):
    ...
    async def check_cooldown(self, ctx: commands.Context, command_name: str) -> bool:
        ...
        if retry_after:
            # Countdown timer нэмэх
            countdown_msg = await ctx.send(f"⏳ Энэ командыг дахин ашиглахын тулд `{retry_after:.1f}` секунд хүлээнэ үү.")

            # Countdown эхлүүлэх
            remaining_time = int(retry_after)
            while remaining_time > 0:
                try:
                    await countdown_msg.edit(content=f"⏳ Энэ командыг дахин ашиглахын тулд `{remaining_time}` секунд хүлээнэ үү.")
                    await asyncio.sleep(1)
                    remaining_time -= 1
                except discord.errors.NotFound:
                    # Мессеж устсан бол countdown зогсоох
                    break
                except Exception:
                    # Бусад алдаа гарсан бол зогсоох
                    break
            
            try:
                await countdown_msg.edit(content="✅ Одоо дахин команд ашиглах боломжтой!")
                # 2 секундын дараа мессежийг устгах
